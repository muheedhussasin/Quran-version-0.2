...
document.getElementById('searchInput').addEventListener('input', function() {
  ...
  const surahs = document.querySelectorAll('.surahs div');

  surahs.forEach(surah => {
    const text = surah.textContent.toLowerCase();
...
const cards = document.querySelectorAll('.blog-card');  // updated selector
...
searchInput.addEventListener('input', function() {
  ...
  cards.forEach(card => {
    ...
    if (ariaLabel.includes(query)) {
      card.style.display = 'flex'; // because your cards are grid/flex
    } else {
...
function filterSurahs(searchTerm) {
  ...
  if(!found) {
    blogContainer.innerHTML = `<p style="text-align:center; width: 100%;">کوئی سورہ نہیں ملا</p>`;
  }
}
